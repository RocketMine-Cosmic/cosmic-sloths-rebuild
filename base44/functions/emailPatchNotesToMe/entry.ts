// One-shot admin utility: emails the FULL verbatim contents of the S6 patch
// note docs (public + staff versions) to the calling admin. Separate from
// emailDocsToMe so the body fits comfortably within Base44's email limits.
...
<h1 style="color:#D946EF;">🌌 S6 Patch Notes — Full Verbatim</h1>
<p>Both patch note docs, full text, ready to read on mobile.</p>

<h2 style="color:#0CA7B8;border-bottom:1px solid #ddd;padding-bottom:4px;margin-top:32px;">📄 S6_PATCH_NOTES.md (public)</h2>
<pre style="white-space:pre-wrap;font-family:'SF Mono',Menlo,monospace;font-size:13px;line-height:1.5;background:#f5f7fa;padding:12px;border-radius:6px;border:1px solid #e1e5eb;">${escapeHtml(PUBLIC_PATCH_NOTES)}</pre>

<h2 style="color:#0CA7B8;border-bottom:1px solid #ddd;padding-bottom:4px;margin-top:32px;">📄 S6_STAFF_PATCH_NOTES.md (staff-only)</h2>
<pre style="white-space:pre-wrap;font-family:'SF Mono',Menlo,monospace;font-size:13px;line-height:1.5;background:#f5f7fa;padding:12px;border-radius:6px;border:1px solid #e1e5eb;">${escapeHtml(STAFF_PATCH_NOTES)}</pre>
...
Deno.serve(async (req) => {
    try {
        ...
        await base44.integrations.Core.SendEmail({
            ...
            to: targetEmail,
            subject: '🌌 S6 Patch Notes — Full Verbatim (public + staff)',
            body: buildHtml(),
        });

        return Response.json({ success: true, sent_to: targetEmail });
    } catch (error) {
        console.error('[emailPatchNotesToMe]', error.message);
